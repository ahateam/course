...
public class CollageService {
	 ...
	 * @param collageName
	 * @param knowledge
	 * @param electId     还要继承一个来自实验室的信息，教师的信息 实验室机构里面应分好从属关系，可暂时把实验室看成一个机构，为高层管理机构。
	 *                    下属有 课程 教师 大纲 实验教室
	 * 
	 * @return
	 */
	public CollageClass addCollage(DruidPooledConnection conn, String collageName, Float knowledge, Byte electId,
			Byte examId) throws Exception {
	 ...
	 * @param collage
	 */
	public CollageClass upCollageClass(DruidPooledConnection conn, Long Id, String CollageName, Float knowledge,
			Byte electId, Byte examId) throws Exception {
		CollageClass c = new CollageClass();
	...
	public List<CollageClass> getCollageClass(DruidPooledConnection conn, Integer count, Integer offset)
			throws Exception {
		return collageRepository.getList(conn, count, offset);
